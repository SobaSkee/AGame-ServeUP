...
usersRouter.get("/", async (_req: Request, res: Response) => {
	try {
		...
		const users = (await collections.users.find({}).toArray()) as DbUser[];
...
usersRouter.get("/:id", async (req: Request, res: Response) => {
	...
	try {
		...
		const user = (await collections.users.findOne(query)) as DbUser;
		if (user) {
...
usersRouter.post("/", async (req: Request, res: Response) => {
	try {
		...
		const newUser = req.body as DbUser;
...
usersRouter.put("/:id", async (req: Request, res: Response) => {
	...
	try {
		...
		const updatedUser: DbUser = req.body as DbUser;
		const query = { _id: new ObjectId(id) };
...
usersRouter.delete("/:id", async (req: Request, res: Response) => {
	...
	try {
		...
		if (result && result.deletedCount) {
			...
			res.status(404).send(`DbUser with id ${id} does not exist`);
		}
	}
	catch (error) {
		var error_msg = (error instanceof Error) ? error.message : "An unknown error occurred";
